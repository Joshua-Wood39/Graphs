...
visited_directions = [(0, "e")]
locked_directions = [(0, "e")]
anomalies = []
room_count = []
current_room = player.currentRoom
room_count.append(current_room)
initial_direction = "n"
direction = None
turn_left = None
...
while len(room_count) < len(roomGraph):
    ...
    for e in exits:
        # print(f"check one: {current_room.id}, {e}")
        # print(f"ch-1 locked: {locked_directions}")
        # print(f"ch-1 visited: {visited_directions}")
        if (current_room.id, e) not in locked_directions and (current_room.id, e) not in visited_directions:
            valid_directions.append(e)

    if len(valid_directions) == 0:
        for e in exits:
            if (current_room.id, e) not in locked_directions:
                valid_directions.append(e)

    # print(f"valid dir: {valid_directions}")
    ...
    if turn_left in valid_directions:
        direction = turn_left
    ...
    if current_room.id == 215 and current_room.id not in anomalies:
        direction = "n"
        anomalies.append(current_room.id)
    if current_room.id == 92 and current_room.id not in anomalies:
        direction = "s"
        anomalies.append(current_room.id)
    if current_room.id == 148 and current_room.id not in anomalies:
        direction = "e"
        anomalies.append(current_room.id)
    if current_room.id == 163 and current_room.id not in anomalies:
        direction = "e"
        anomalies.append(current_room.id)
    ...
    current_room = player.currentRoom
    if current_room not in room_count:
        room_count.append(current_room)
    ...
    # print(f"backtrack direction: {behind_me}")

    if (current_room.id, behind_me) in visited_directions:
        locked_directions.append((current_room.id, behind_me))
    else:
        visited_directions.append((current_room.id, behind_me))

    # check if exits have been visited/locked
    # pick valid exit
    # add room # and direction to visited_directions
    # update room
    # if direction came from is in visited_directions...
    # add direction came from to locked_directions
    # add direction came from to visited_directions
